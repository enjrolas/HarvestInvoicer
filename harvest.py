...
class User(HarvestItemBase):
    ...
    def unbilled_entries(self,start,end):
        return self.harvest._unbilled_time_entries( '%s/%d/' % (self.base_url, self.id), start, end )
...
class Harvest(object):
    ...
    def _unbilled_time_entries(self,root,start,end):
        url = root + 'entries?from=%s&to=%s&only_unbilled=yes' % (start.strftime('%Y%m%d'), end.strftime('%Y%m%d'))
